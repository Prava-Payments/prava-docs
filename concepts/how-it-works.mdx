---
title: 'How Prava Works'
description: 'The lifecycle behind every Prava payment — from intent to a completed checkout.'
...
```
   Who pays                 Prava                        Merchant
 ─────────────    ──────────────────────────     ────────────────────
  proposes a  ──▶  collect the card securely  ──▶  complete the checkout
  purchase         apply the owner's limits
                   issue a one-time credential
...
<Card title="No raw card exposure" icon="shield-halved">
  The card is entered in Prava's secure surface and tokenized. Apps and agents never see it.
</Card>
...
|---|---|---|---|
| Runs in | Your web app (SDK) | Your backend (API) | An AI agent — CLI today, MCP connector soon |
