...
<Note>
  ...
  endpoint for the normalized outcome, `transactions[].card_id`, and quote lifecycle context in
  `merchant_res`; token, CVV, expiry, and line-item credentials are not returned, and quote callers
