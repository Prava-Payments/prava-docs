...
## The non-quote checkout lifecycle you drive

For a non-quote checkout, a complete payment is **three API calls and one hand-off**.
Everything else on this page is what Prava does for you in between:
...
<Note>
  In a quote-backed checkout, Prava performs the merchant checkout. Poll the same payment-result
  endpoint for the normalized top-level `status`, `error`, and `transactions[].card_id`.
  When merchant lifecycle data exists, `merchant_res` adds its status and optional merchant order ID;
  token, CVV, expiry, and line-item credentials are not returned, and quote callers
  do not call the public Report Status endpoint.
</Note>
...
**Top-level payment-result statuses:**

| Status | Meaning |
|--------|---------|
| `pending` | Session created; no transaction started yet |
| `processing` | Payment in flight — a non-quote checkout is authenticating or tokenizing, or a quote-backed merchant checkout is not yet terminal |
| `awaiting_result` | Non-quote checkout credentials issued; awaiting your [status report](/api-reference/report-status) |
| `completed` | Payment authorized and completed |
| `failed` | Payment failed (declined, timeout, or error) |

In a quote-backed result, `transactions[].status` can remain `awaiting_result` while the top-level
status is `processing`, `completed`, or `failed`; the top-level status is the normalized checkout
outcome.
...
### Payment tokens — the non-quote checkout credential you receive

Against an active mandate, Prava generates **payment tokens**: a virtual card number (PAN), expiry,
and CVV, scoped to the mandate constraints. These are what step 3 hands back for non-quote checkouts:
