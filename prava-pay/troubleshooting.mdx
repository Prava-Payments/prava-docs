...
<Accordion title="Tokenization failed.">
  ...
  failing, contact [support](mailto:developers@prava.space).
</Accordion>
...
<Warning>
  The token and cryptogram from `sessions poll` are **single-use and short-lived**. If you wait too
  long to check out, mint a fresh session — don't try to reuse old credentials.
</Warning>
...
<Accordion title="request failed: timed out — the merchant checkout can be slow; try again">
  ...
  simply retry (the CLI retries once by default; raise it with `--retries`). For **checkout**, see the
...
<Accordion title="Checkout quota exceeded">
  ...
  raise the limit or contact [support](mailto:developers@prava.space). See [Guardrails](/concepts/guardrails).
</Accordion>
...
The result of a checkout is read from the payment itself, not just the network response — so the
status you see is the real one.
...
For anything else, reach us at [developers@prava.space](mailto:developers@prava.space).
