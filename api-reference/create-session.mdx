...
description: 'Create a payment session for a customer and order — standard checkout or mandate setup.'
icon: 'plus'
...
<Tabs>
  ...
  <Tab title="Mandate setup">
    Include a `mandate_setup` block with `intent: "mandate_setup"`. The session is **authorize-only**: it returns an `iframe_url` for passkey approval, and completing setup issues **no** payment credentials. Charge later with [Charge a Mandate](/api-reference/mandate-charge). There is no separate `POST /v1/mandates` create endpoint — a [mandate](/concepts/mandates) is always set up through this endpoint.
  </Tab>
</Tabs>

## Mandate setup

**Mandate creation is part of the Session APIs — this endpoint is how mandates are created.** Send a `mandate_setup` block (see [Body](#body)) with `intent: "mandate_setup"` and the session becomes authorize-only: the response carries an `iframe_url` where the owner approves **once** with their passkey. Completing setup does not issue payment credentials. Once the mandate is `active`, charge it with [Charge a Mandate](/api-reference/mandate-charge) within its approved terms — no further passkey needed.
...
<Note>
  **Choosing a card source.** Omit `card` to let the cardholder select or enter a card in the collection surface. Send `card.card_id` to pre-select an already-enrolled card, or `card.vault_ref_id` (a Skyflow UUID) for the vault-ref flow. Send **exactly one** — if both are present, `card_id` wins.
  After collection, [Get Payment Result](/api-reference/get-payment-result) returns the card bound to
  each transaction as `transactions[].card_id`.
</Note>
...
<ParamField body="purchase_context" type="object" required>
  ...
  - **From a quote** — send `quote: true` and the opaque `quote_id` (`qte_…`) returned when the quote
    was created. Merchant and product details come from that quote, so nothing else is needed.
  - **Custom** — omit `quote` (or send `false`) and describe the merchant and products yourself in `custom`.

  <Expandable title="purchase_context properties">
    <ParamField body="quote" type="boolean" default={false}>
      Set `true` to build the session from an existing quote. When `true`, pass `quote_id` and
      nothing else — `custom` must be omitted. When `false` or absent, `custom` is required and
      `quote_id` must be omitted.
    </ParamField>

    <ParamField body="quote_id" type="string">
      **Required when `quote` is `true`.** The opaque public quote id returned by quote creation
      (for example, `qte_…`). The quote carries the merchant, products, and locked price.
      Create the session before the quote expires; its expiry is separate from the session's
      lifetime. `total_amount` and `currency` must match the quote's final total.
      Do not send a merchant checkout-session id.
    </ParamField>

    <ParamField body="custom" type="object[]">
      **Required when `quote` is not `true`.** Exactly one entry — multi-merchant sessions are not
      yet supported.
...
<ParamField body="sandbox_simulation" type="object">
  Optional, sandbox-only payment success simulation for quote-backed checkout. The exact shape
  is `{ "stage": "payment", "outcome": "success" }`; both fields are required and additional
  fields are rejected. Custom checkout, production, and mandate-setup sessions cannot use it.
  Normal card collection and verification still run. An eligible terminal payment result can
  complete the sandbox lifecycle while `observed_result` retains the actual outcome.
  See [Sandbox simulation](/api-reference/testing#simulate-successful-payment-completion).
</ParamField>
...
| Status | Code | Cause |
|--------|------|-------|
| 400 | `VAL_2001` | Validation error (missing/invalid fields; `user_id`/`user_email` missing on a merchant session; `quote: true` without a `quote_id`; `custom` missing when `quote` is not true). `details` names the fields. |
...
| 400 | `SANDBOX_SIMULATION_NOT_ALLOWED` | Simulation requested outside sandbox, for a custom checkout, or for mandate setup |
| 401 | `AUTH_1001` | Invalid API key |
| 401 | `AUTH_1002` | Missing or invalid Authorization header |
| 409 | `QUOTE_SESSION_MISMATCH` | Session amount or currency differs from the quote's final total |
| 410 | `QUOTE_EXPIRED` | The quote expired before session creation |
