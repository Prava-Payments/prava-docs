...
<ParamField body="purchase_context" type="object" required>
  ...
  - **From a quote** — send `quote: true` and a `quote_id`. Merchant and product details come from the
    [quote](/integration/ucp), so nothing else is needed.
  ...
  <Expandable title="purchase_context properties">
    ...
    <ParamField body="quote_id" type="string">
      **Required when `quote` is `true`.** The id of the quote from the
      [shopping flow](/prava-pay/shopping) (`checkout-session-id`, e.g. `ches_…`). The quote carries the
      merchant, products, and locked price — note that quotes expire (~15 minutes).
    </ParamField>
