---
title: 'Charge a Mandate'
description: 'Mint single-use card credentials against an active mandate — no passkey.'
icon: 'bolt'
openapi: '/api-reference/openapi.json POST /v1/mandates/{id}/charge'
...
<ParamField body="purchase_context" type="object[]">
  Optional per-charge product details (exactly one entry, same shape as the `purchase_context.custom` entries in [Create Session](/api-reference/create-session)). Omit to reuse the mandate's setup context. Currency always comes from the mandate, never the charge.
</ParamField>
