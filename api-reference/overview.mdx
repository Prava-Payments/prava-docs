...
```
X-Response-ID: 8b74ce32-4682-4b17-b51f-d57808f99061
```
...
A non-quote payment is **three API calls and one non-API step**. Quote-backed checkouts use the same
session and polling endpoints, but Prava completes the merchant checkout for you:
...
<Step title="Poll the result — GET /v1/sessions/{sessionId}/payment-result">
  For a non-quote checkout, this returns one-time `token` and `dynamic_cvv` credentials. For a
  quote-backed checkout, it returns a credential-free transaction summary with the card actually
  used and the normalized outcome in the top-level `status` and `error`. When merchant lifecycle
  data exists, `merchant_res` also provides its normalized status and optional merchant order ID.
...
<Step title="Report the outcome — POST /v1/sessions/{sessionId}/report-status">
  For non-quote checkouts, tell Prava whether checkout succeeded (`APPROVED`) or failed (`DECLINED`)
  so transaction records and the card network stay in sync. Quote callers do not call this public
  endpoint; poll their normalized payment result instead. → [Report Status](/api-reference/report-status)
</Step>
</Steps>
...
<CardGroup cols={2}>
<Card title="Enroll Card" icon="credit-card" href="/api-reference/enroll-card">
  Provision a card only, or create an interactive card-and-passkey enrollment session
</Card>
<Card title="Card Enrollment Status" icon="rotate" href="/api-reference/get-card-enrollment-status">
  Poll an interactive card-enrollment session without exposing payment credentials
</Card>
