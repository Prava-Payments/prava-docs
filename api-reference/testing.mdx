...
The response contains `transactions[].card_id` and `merchant_res`, but no token, CVV, expiry, or
line-item credentials; Prava performs the merchant checkout, so you do not call Report Status.
