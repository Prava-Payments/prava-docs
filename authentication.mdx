...
*   Passed during SDK initialization: `new PravaSDK({ publishableKey: 'pk_live_xxx' })`
...
After creating a session via the backend (`POST /v1/sessions`), the returned `session_token` authenticates all subsequent operations within that session (card collection, transactions, FIDO authentication). Session tokens are:
...
[developers@prava.space](mailto:developers@prava.space) when you're ready to go live.
</Note>

## Domain Allowlisting

Each merchant account has a list of **allowed domains**. The SDK iframe and session middleware validate that requests originate from an allowed domain. Add your frontend domains in the [Prava Dashboard](https://dashboard.prava.space).
...
Prava supports webhooks for asynchronous status updates (e.g., payment completions, disputes, card status changes).

*   Configure your webhook URL during merchant onboarding.
*   Each webhook event includes a signature header — verify it using the SDK helper or the `webhook_secret` provided during merchant creation.
