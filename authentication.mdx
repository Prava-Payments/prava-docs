...
Each merchant account has a list of **allowed domains**. The SDK iframe (the embedded frame that renders the card form) and session middleware validate that requests originate from an allowed domain. Add your frontend domains in the [Prava Dashboard](https://dashboard.prava.space).
