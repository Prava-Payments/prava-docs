---
title: 'FAQs & Concerns'
description: 'Common questions about integrating with Prava.'
icon: 'question'
---
...
A: The card network will decline the transaction. Payment tokens are merchant-locked — they can only be used at the merchant specified in the mandate. This is enforced at the network level, not just by Prava.

**Q: Can AI agents make purchases without user approval?**

A: No. Every intent requires explicit user authentication via Passkey (biometric/device confirmation). The agent cannot bypass this step. Once a mandate is approved, the agent can invoke it within the mandate's constraints (amount, merchant, frequency) — but the user approved those constraints upfront.
...
A: Prava can already reach Shopify stores through [UCP](/integration/ucp) and the [Browser Harness](/integration/browser-harness) — no setup needed on your end. If you'd like a direct integration, the **Prava Shopify app** is available by **invite only**. Contact **developers@prava.space** to request merchant onboarding.
