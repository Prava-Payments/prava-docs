...
works out the true final total, and pays with the one-time tokenized card — so an AI application
completes the purchase without ever touching the raw card or the merchant's checkout code.
...
<Step title="One clean attempt">
  A checkout runs as a single, tracked flow — not a blind retry loop — so a purchase either completes
  and returns an order, or fails cleanly for you to re-quote.
</Step>
...
```
  UCP quote            →     Browser Harness         →     Result
 ───────────               ───────────────────           ───────────
  price the item            confirm true total,           order id + status
                            pay with one-time token
```
