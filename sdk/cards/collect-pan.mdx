...
<ParamField path="options" type="CollectPANOptions" required>
  ...
  <Expandable title="properties">
    ...
    <ParamField path="onSuccess" type="(result: CollectPANResult) => void">
      Called when card provisioning succeeds. In a `full_enrollment` session, passkey work may
      still be in progress; use `onEnrollmentComplete` for the final enrollment result.
    </ParamField>

    <ParamField path="onEnrollmentComplete" type="(payload: CardEnrollmentCompletePayload) => void">
      Called only for an enrollment-only session after the card and passkey are ready. The payload
      contains `session_id`, `card_id`, `status: "completed"`,
      `outcome: "fully_enrolled"`, `passkey: "registered" | "existing"`, and
      `callback_url`.
    </ParamField>

    <ParamField path="onEnrollmentCancelled" type="(payload: { session_id: string; card_id: string | null }) => void">
      Called when the cardholder cancels an enrollment-only session. `card_id` can be non-null
      when card provisioning completed before cancellation.
    </ParamField>
...
## Full card-enrollment sessions

When `collectPAN()` receives a session created by
[Enroll Card](/api-reference/enroll-card) with `enrollment_mode: "full_enrollment"`, its normal
`onSuccess` callback still reports the earlier card-provisioning milestone. Keep the SDK and iframe
mounted until `onEnrollmentComplete` or `onEnrollmentCancelled`.
...
void prava.collectPAN({
  ...
  container: '#card-form',
  onSuccess: (card) => console.log('Card provisioned', card.enrollmentId),
  onEnrollmentComplete: (result) => {
    console.log('Full enrollment complete', result.card_id, result.passkey);
  },
  onEnrollmentCancelled: (result) => {
    console.log('Enrollment cancelled', result.card_id);
  },
  onError: (error) => console.error(error.code, error.message),
});
```

Use the callback for frontend coordination, but confirm the durable server-side result with
[Get Card Enrollment Status](/api-reference/get-card-enrollment-status). A retryable iframe error
does not require a new SDK call: leave the iframe mounted while the cardholder selects **Try again**.
...
<CardGroup cols={2}>
<Card title="Enroll Card (API)" icon="credit-card" href="/api-reference/enroll-card">
  Create a provision-only or full card-enrollment session
</Card>
