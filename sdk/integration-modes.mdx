...
description: 'Two ways to collect a card from one session — embedded (SDK) or hosted (redirect).'
...
<Note>
  Enrollment-only sessions created by
  [`POST /v1/sessions/enroll-card`](/api-reference/enroll-card) use these same two presentation
  modes. Their backend result comes from
  [Get Card Enrollment Status](/api-reference/get-card-enrollment-status), not Payment Result.
</Note>
...
| How you get the result | Payments: `onSuccess`; full enrollment: `onEnrollmentComplete` | Payments: `callback_url`; full enrollment: callback or final screen |
...
For full enrollment, your backend confirms either UI result by polling
...
<Warning>
  ...
  A hosted **card-enrollment-only** session is the exception: its callback is optional, and without
  one Prava keeps the completion screen visible so the cardholder can safely close the window.
</Warning>
